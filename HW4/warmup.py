...
#This piece of code will write the integers in a list to a binary file. It does so by using structs from C language.

#By default, C types are represented in the machine's native format and byte order.
#Alternatively, the first character of the format string can be used to indicate the byte order,
#size and alignment of the packed data.
#Native byte order is big-endian or little-endian, depending on the host system. 
#For example, Intelx86 and AMDx86 are little-endian.
...
def write_data_to_binary_file(item_list, file_name):
    with open(file_name, "wb") as file_object:
        for item in item_list:
            ...
            #The < sign ensures that our conversion is to the little-endian format. 
            #i is for int and q is for long long.
